...
def login():
    if request.method == 'POST':
        ...
        if usuarios.find_one(usuario):
            ...
            email = yagmail.SMTP("sarriaarceanamaria18@gmail.com", open(".password").read(), encoding='UTF-8')
            mensaje = f"Se informa que el usuario {username} ha ingresado al sistema"
            thread = threading.Thread(target=enviarCorreo, args=(email, "sarriaarceanamaria18@gmail.com", "Reporte ingreso al sistema usuario", mensaje))
            thread.start()
            return redirect("/listarProductos")
        
        return render_template("frmLogin.html", mensaje="Credenciales de ingreso no válidas")
