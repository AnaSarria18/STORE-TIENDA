...
def listar_productos():
    if "user" in session:
        try:
            ...
            return render_template("listarProductos.html", productos=lista_productos)
        except pymongo.errors.PyMongoError as error:
            return render_template("listarProductos.html", mensaje=str(error))
    return render_template("frmLogin.html", mensaje="Debe primero ingresar con sus credenciales")
...
def _agregar_producto():
    try:
        ...
        if not existe_producto(codigo):   
            ...
            return render_template("frmAgregarProducto.html", mensaje="Problemas al agregar el producto.")
        return render_template("frmAgregarProducto.html", mensaje="Ya existe un producto con ese código")
    except pymongo.errors.PyMongoError as error:
        return render_template("frmAgregarProducto.html", mensaje=str(error))
    ...
    if "user" in session:
        ...
        try:
            ...
            if not productos.find_one({"codigo": codigo, "_id": {"$ne": id}}):
                ...
                return render_template("frmActualizarProducto.html", producto=producto, mensaje="Producto ya existe con ese código.")
        except pymongo.errors.PyMongoError:
            return redirect("/listarProductos")
        except Exception as e:
            
            return render_template("frmActualizarProducto.html", mensaje=str(e), producto=producto)
